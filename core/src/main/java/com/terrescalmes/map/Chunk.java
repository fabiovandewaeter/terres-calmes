...
public class Chunk {
    ...
    public Chunk(Vector2 position) {
        ...
        if (position.x == 0 && position.y == 0) {
            structures.add(
                    new Structure(TextureManager.getInstance().getTextureRegion("crystals", 14, 2), new Vector2(1, 0),
                            0.5f, 0.5f));
        }
        if (position.x == 0 && position.y == 0) {
            structures.add(
                    new Structure(TextureManager.getInstance().getTextureRegion("trees", 0, 0), new Vector2(3, 0),
                            0.5f, 1.5f));
        }
    }
    ...
    private void generateTiles() {
        for (int row = CHUNK_SIZE - 1; row >= 0; row--) {
            for (int col = CHUNK_SIZE - 1; col >= 0; col--) {
                ...
                textureRegion = textureManager.getTextureRegion("tiles", 0, 0);
